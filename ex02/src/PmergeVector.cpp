...
// ========================================================================
PmergeVector::PmergeVector(int ac, char **av)
{
	if (ac < 2)
	{
...
PmergeVector::PmergeVector(const PmergeVector &src)
{
	if (this != &src)
	{
...
bool PmergeVector::isDigit(const std::string &str) const
{
	if (str.empty())
		return false;
...
bool PmergeVector::isValidInt(const std::string &str) const
{
...
bool PmergeVector::is_sort(std::vector<int> &vec)
{
...
size_t PmergeVector::createPairs()
{
...
void PmergeVector::makePairs(std::vector<Pair> &pairs, size_t pairSize)
{
	pairs.clear();
	size_t size = _numbers.size();
	if (pairSize < 1)
...
void PmergeVector::recursivePairs(std::vector<Pair> &pairs, size_t pairSize)
{
	if (pairSize < 1)
		return;
...
void PmergeVector::insert_pair(std::vector<Pair> &main, const Pair &element, int search_limit)
{
...
size_t PmergeVector::find_partner_position(const Pair &pend_element)
{
...
bool PmergeVector::compare_pairs(const Pair &a, const Pair &b)
{
	return a.pair.back() < b.pair.back();
}

int PmergeVector::jacobstahl_numbers(int n)
{
	if (n == 0)
		return 0;
	if (n == 1)
...
void PmergeVector::displayNumbers() const
{
...
void PmergeVector::displayRest() const
{
...
void PmergeVector::displayMain() const
{
	if (_main.empty())
	{
...
void PmergeVector::displayPend() const
{
	if (_pend.empty())
	{
...
void PmergeVector::displayPair(const std::vector<Pair> &pairs) const
{
...
std::vector<int> PmergeVector::getNumbers() const
{
	return _numbers;
}